...
void DoInitStuff()
{
    ...
    if (!bNetworkInited && !bDebug)
    {
        ...
        if (pSettings)
        {
            ...
            bNetworkInited = true;
            FLog("DoInitStuff end - Connected to %s:%d", pSettings->Get().szHost, pSettings->Get().iPort);
        }
        else
        {
            FLog("ERROR: pSettings is null! Cannot connect.");
        }
    }
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    g_pszStorage = strdup("/storage/emulated/0/Android/data/com.samp.mobile/files/");
    FLog("Storage path: %s", g_pszStorage);
